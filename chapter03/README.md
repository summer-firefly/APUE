...
UNIX系统支持在不同进程间共享打开的文件，也就是说多个进程可以同时打开相同的文件，在系统内核中，有一些与文件相关的重要内核数据结构：

- 进程控制块PCB(struct task_struct)
- 文件描述符表
- 文件表
- v节点(v-node)结构
...
![文件相关内核数据结构](./png/file_struct.png)

> 进程控制块(PCB)

UNIX系统下的进程控制块为`struct task_struct`，进程控制块主要用于管理进程，进程控制块内部包含许多重要结构，每一个进程都有属于自己的独立的进程控制块

> 文件描述符表

文件描述符表是进程控制块内部重要的数据结构，用于将进程与文件之间建立关联，文件描述符表实际上是一个文件指针数组，文件描述符作为数组的下标，每一个文件描述符对应一个文件指针，文件指针指向文件表
...
文件描述符、文件指针与文件表项的对应关系是1对1的，文件表项属于进程内部的结构，进程之间不共享文件表项

> V节点表项

每一个打开的文件在内核中都有一个v-node结构，文件表项中的v节点指针就指向文件的v-node结构，v-node结构中包含以下内容：

- i-node(索引节点)，i-node中包含文件的所有者、文件长度、指向文件实际数据块在磁盘上所在位置的指针
- 文件类型
- 对文件进行各种操作的函数指针

v节点表项与文件强相关，所有进程公用v节点表项，进程控制块、文件描述符表和文件表项与进程强相关，每一个进程单独拥有这些结构。

如果2个独立的进程打开同一文件，示例图如下：

![进程打开同一文件](./png/open_same_file.png)

如果一个进程2次打开同一个文件，会使用不同的文件表项，但是它们的v节点指针指向相同的v节点表项

[进程2次打开同一个文件](./src/open_same_file.c)
